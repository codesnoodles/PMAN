...
from .attention import LocalAttention, ChannelAttention, LocalpatchAttention
...
class CrossConformer_big_change(nn.Module):

    def __init__(self, in_channels, out_channels):
        super(CrossConformer_big_change, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.mid_channels = in_channels // 2
        self.input = nn.Sequential(
        ...
        self.ca1 = ChannelAttention(channels=self.mid_channels)
        self.ca2 = ChannelAttention(channels=self.mid_channels)
        self.tpa1 = LocalpatchAttention(in_channels=self.mid_channels,
                                        reduction=1,
                                        pool_t=1,
                                        pool_f=257,
                                        add_input=True)
        self.tpa2 = LocalpatchAttention(in_channels=self.mid_channels,
                                        reduction=1,
                                        pool_t=1,
                                        pool_f=257,
                                        add_input=True)
        self.fpa1 = LocalpatchAttention(in_channels=self.mid_channels,
                                        reduction=1,
                                        pool_t=126,
                                        pool_f=1,
                                        add_input=True)
        self.fpa2 = LocalpatchAttention(in_channels=self.mid_channels,
                                        reduction=1,
                                        pool_t=126,
                                        pool_f=1,
                                        add_input=True)
    ...
    def forward(self, x):
        ...
        x_tf = x_f + x_t
        x_tfw = self.aff1(x_tf)
        x_out = x_f * x_tfw + (1 - x_tfw) * x_t
        x_out = self.ca1(x_out)
        x_out = self.tpa1(x_out)
        x_out = self.fpa1(x_out)
        ...
        y_tf = y_f + y_t
        y_tfw = self.aff2(y_tf)
        y_out = y_f * y_tfw + (1 - y_tfw) * y_t
        y_out = self.ca2(y_out)
        y_out = self.tpa2(y_out)
        y_out = self.fpa2(y_out)

        output = self.output(y_out)
...
if __name__ == '__main__':
    ...
    # model = CrossConformer_big(64, 64)
